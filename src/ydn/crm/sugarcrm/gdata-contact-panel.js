...
goog.provide('ydn.crm.su.GDataContactPanel');
 ...
 */
ydn.crm.su.GDataContactPanel = function(m) {
  goog.base(this, m);
...
goog.inherits(ydn.crm.su.GDataContactPanel, ydn.crm.su.SyncPanel);


/**
 * @define {boolean} debug flag
 */
ydn.crm.su.GDataContactPanel.DEBUG = true;


/**
 * @override
 */
ydn.crm.su.GDataContactPanel.prototype.renderContent = function() {
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.onContentClick_ = function(ev) {
  ...
  if (el.tagName == 'BUTTON') {
    ...
    if (name == 'link') {
      ...
      this.link_(gdata_id, /** @type {ydn.crm.su.ModuleName} */(mn), record_id);
    }
  } else {
    ...
    if (name) {
      ...
      this.import_(id, /** @type {ydn.crm.su.ModuleName} */ (name));
    }
  }
 ...
 * @override
 */
ydn.crm.su.GDataContactPanel.prototype.renderToolbar = function() {
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.onOrderChanged_ = function(e) {
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.onDirChanged_ = function(e) {
  console.log(e);
 ...
 * @protected
 */
ydn.crm.su.GDataContactPanel.prototype.refreshFooter = function() {
  var query = {
    'kind': ydn.gdata.Kind.M8_CONTACT
  };
 ...
 * @typedef {{
 *   score: number,
 *   record: ydn.crm.su.Record
 * }}
 */
ydn.crm.su.GDataContactPanel.SimilarityResult;


/**
 * @param {string} domain
 * @param {Array<ydn.crm.su.GDataContactPanel.SimilarityResult>} arr sorted
 * array by score.
 * @param {SugarCrm.ContactSimilarityResult} res result to put into arr.
 * @private
 */
ydn.crm.su.GDataContactPanel.enrichFor_ = function(domain, arr, res) {
  for (var i = 0; i < res.result.length; i++) {
    ...
    if (ex_idx == -1) {
      var mn = /** @type {ydn.crm.su.ModuleName} */ (res.module);
      var obj = {
        score: score,
        record: new ydn.crm.su.Record(domain, mn, record)
      };
 ...
 * @return {Array<ydn.crm.su.GDataContactPanel.SimilarityResult>}
 * Results for for each module. Results are order by score with higher score
 * comes first.
 */
ydn.crm.su.GDataContactPanel.enrich = function(domain, results) {
  var arr = [];
  for (var i = 0; i < results.length; i++) {
    ...
    ydn.crm.su.GDataContactPanel.enrichFor_(domain, arr, res);
  }
  return arr;
};
 ...
 */
ydn.crm.su.GDataContactPanel.importMenuItems = [{
  label: 'Accounts',
  name: 'Accounts',
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.refreshEntry_ = function(id) {
 ...
 * @param {Element} el
 * @param {Array<ydn.crm.su.GDataContactPanel.SimilarityResult>} results
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.renderRecordBySimilarity_ = function(
    el, results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    icon.textContent = ydn.crm.su.toModuleSymbol(m_name);
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.renderRecordByLink_ = function(
    el, entry, r) {
  ...
  var record = new ydn.crm.su.Record(this.model.getDomain(), mn, r);
  var sec_el = this.secondary_templ_.cloneNode(true).firstElementChild;
  var label = sec_el.querySelector('[name=label]');
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.renderEntry_ = function(el, entry) {
  var primary = el.querySelector('.primary');
  var secondary = el.querySelector('.secondary');
  primary.innerHTML = '';
  secondary.innerHTML = '';
  ...
  return ch.send(ydn.crm.Ch.Req.SYNC_QUERY, q).addCallbacks(function(arr) {
    ...
    if (arr[0]) {
      ...
      sch.send(ydn.crm.Ch.SReq.QUERY_SIMILAR, ce).addCallbacks(function(arr) {
        ...
        var results = ydn.crm.su.GDataContactPanel.enrich(domain, arr);
 ...
 * @private
 */
ydn.crm.su.GDataContactPanel.prototype.renderNextEntry_ = function(arr, idx) {
  if (!arr[idx]) {
 ...
 * @override
 */
ydn.crm.su.GDataContactPanel.prototype.refreshContent = function() {
  var ul = this.root.querySelector('.content UL');
  ul.innerHTML = '';
