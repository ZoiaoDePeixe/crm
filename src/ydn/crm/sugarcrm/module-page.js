...
ydn.crm.su.ui.ModulePage.prototype.createDom = function() {
  ...
  var m_menu = new goog.ui.Menu(dom);
  var modules = model.listModules();
  for (var i = 0; i < modules.length; i++) {
    var m_item = new goog.ui.CheckBoxMenuItem(modules[i], modules[i], dom);
    if (modules[i] == this.name) {
      m_item.setChecked(true);
    }
    m_menu.addChild(m_item, true);
  }
  var m_btn = new goog.ui.MenuButton(symbol, m_menu, null, dom);
  ...
  var filter = new goog.ui.MenuButton('Filter', f_menu, null, dom);
  ...
  var sort = new goog.ui.MenuButton('Sort', s_menu, null, dom);
